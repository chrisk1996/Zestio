...
import FurnitureLibraryPanel from '@/components/floorplan/FurnitureLibraryPanel';
...
export default function FloorPlanPage() {
  ...
  const [selectedFurnitureItem, setSelectedFurnitureItem] = useState<FurnitureDragItem | null>(null);
  ...
  }, [selectedId, selectedType, selectedFurnitureId, walls, rooms, setWalls, setRooms, deleteFurniture, rotateFurniture, handleUndo, handleRedo]);
  ...
  const handleFileUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = async (event) => {
      try {
        ...
        if (data.walls) {
          setWalls(data.walls.map((w, i) => ({
            id: `wall-${i}`, x1: w.start[0] * 20, y1: w.start[1] * 20,
            x2: w.end[0] * 20, y2: w.end[1] * 20, type: w.type,
            thickness: w.type === 'exterior' ? 30 : 15,
          })));
        }
        if (data.rooms) {
          setRooms(data.rooms.map((r, i) => ({
            id: `room-${i}`, name: r.name, type: r.type,
            points: [r.x * 20, r.y * 20, (r.x + r.width) * 20, r.y * 20,
              (r.x + r.width) * 20, (r.y + r.height) * 20, r.x * 20, (r.y + r.height) * 20],
          })));
        }
      } catch {
  ...
  const handleNew = useCallback(() => {
    if (confirm('Clear all and start new project?')) {
      setWalls([]); setRooms([]); setDoors([]); setWindows([]); clearAllFurniture();
    }
      ...
      <div className="flex h-[calc(100vh-5rem)]">
        ...
        <div className="flex-1 flex flex-col">
          <div className="flex border-b border-slate-200 bg-white" data-tutorial="views">
            <button onClick={() => setViewMode('2d')} className={`px-6 py-3 text-sm font-semibold transition-all ${viewMode === '2d' ? 'bg-blue-600 text-white' : 'text-slate-600 hover:bg-slate-50'}`}>2D Editor</button>
            <button onClick={() => setViewMode('3d')} className={`px-6 py-3 text-sm font-semibold transition-all ${viewMode === '3d' ? 'bg-blue-600 text-white' : 'text-slate-600 hover:bg-slate-50'}`}>3D View</button>
            <button onClick={() => setViewMode('split')} className={`px-6 py-3 text-sm font-semibold transition-all ${viewMode === 'split' ? 'bg-blue-600 text-white' : 'text-slate-600 hover:bg-slate-50'}`}>Split</button>
            <div className="flex-1" />
            <button onClick={() => fileInputRef.current?.click()} className="px-4 py-2 m-2 bg-emerald-600 text-white rounded-lg text-sm font-semibold hover:bg-emerald-700">
              <span className="material-symbols-outlined text-sm mr-1">upload</span> Upload
            </button>
          ...
          <div className={`flex-1 flex ${viewMode === 'split' ? 'divide-x divide-slate-200' : ''}`}>
            {(viewMode === '2d' || viewMode === 'split') && (
              <div className={viewMode === 'split' ? 'w-1/2' : 'flex-1'}>
                <FloorPlanCanvas2D walls={walls} rooms={rooms} doors={doors} windows={windows}
                  onWallsChange={setWalls} onRoomsChange={setRooms}
                  onSelectionChange={(id, type) => { setSelectedId(id); setSelectedType(type); if (id) setSelectedFurnitureId(null); }} />
              </div>
            )}
            {(viewMode === '3d' || viewMode === 'split') && (
              <div className={viewMode === 'split' ? 'w-1/2' : 'flex-1'}>
                <FloorPlan3DViewer floorPlanData={{
                  rooms: rooms.map(r => ({ name: r.name, x: r.points[0] / 20, y: r.points[1] / 20,
                    width: (r.points[4] - r.points[0]) / 20, height: (r.points[5] - r.points[1]) / 20, type: r.type })),
                  walls: walls.map(w => ({ start: [w.x1 / 20, w.y1 / 20] as [number, number],
                    end: [w.x2 / 20, w.y2 / 20] as [number, number], type: w.type })),
                  doors: [], windows: [], totalArea: 0, bedroomCount: 0, bathroomCount: 0,
                }} furniture={placedFurniture} selectedPieceId={selectedPieceId}
                  onFurniturePlace={(pos) => { if (!selectedFurnitureItem) return;
                    setPlacedFurniture(prev => [...prev, { id: `${selectedFurnitureItem.id}-${Date.now()}`,
                      furniture: { id: selectedFurnitureItem.id, name: selectedFurnitureItem.name, category: selectedFurnitureItem.category, dimensions: { width: selectedFurnitureItem.width, height: 1, depth: selectedFurnitureItem.height }, color: selectedFurnitureItem.color, } as FurnitureItem, position: [pos[0], 0, pos[2]], rotation: 0, scale: 1 }]);
                  }} onFurnitureSelect={setSelectedPieceId} cameraPreset={cameraPreset} lightingMode={lightingMode} />
              </div>
            )}
          </div>
        ...
        <div data-tutorial="properties">
          <PropertiesPanel selectedId={selectedId} selectedType={selectedType === 'furniture' ? null : selectedType as 'wall' | 'room' | 'door' | 'window' | null}
